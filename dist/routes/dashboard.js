...
async function resolveClinicsInScope(user, requestedClinicId) {
...
async function resolveEncounterMatchers(user, requestedClinicId, requestedDate) {
...
export async function registerDashboardRoutes(app) {
    ...
    app.get("/dashboard/office-manager", { preHandler: officeGuard }, async (request) => {
        ...
        const statusMap = Object.fromEntries(Object.values(EncounterStatus).map((status) => [
            status,
            statusCounts.find((row) => row.currentStatus === status)?._count._all || 0
        ]));
        const alertMap = Object.fromEntries(Object.values(AlertLevel).map((severity) => [
            severity,
        ...
        return {
            ...
            inProgressCount: statusMap.Lobby + statusMap.Rooming + statusMap.ReadyForProvider + statusMap.Optimizing + statusMap.CheckOut
        };
    });
    ...
    app.get("/dashboard/revenue-cycle", { preHandler: requireRoles(RoleName.RevenueCycle, RoleName.Admin) }, async (request) => {
        const query = dashboardQuerySchema.parse(request.query);
        const matchers = await resolveEncounterMatchers(request.user, query.clinicId, query.date);
        ...
        const closureTypeCounts = closureCounts.reduce((acc, row) => {
            if (!row.closureType)
                return acc;
