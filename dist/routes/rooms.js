import { AlertInboxKind, RoleName, RoomChecklistKind, RoomEventType, RoomHoldReason, RoomIssueStatus, RoomIssueType, RoomOperationalStatus, TaskSourceType } from "@prisma/client";
import { z } from "zod";
        ...
        const result = await prisma.$transaction(async (tx) => {
            const issue = await tx.roomIssue.create({
                data: {
                    ...
                    metadataJson: dto.metadata ? dto.metadata : undefined,
                    createdByUserId: request.user.id
                }
            });
            ...
            if (dto.placesRoomOnHold) {
                ...
                await tx.roomOperationalEvent.create({
                    data: {
                        ...
                        note: dto.title,
                        metadataJson: { issueId: issue.id, taskId: task.id },
                        createdByUserId: request.user.id
                    }
                });
            }
