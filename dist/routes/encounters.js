...
const cancelSchema = z
    ...
    reason: (value.reason || value.closureType || ""),
    note: value.note || value.closureNotes
}));
function withStatusAlias(encounter) {
    return {
        ...encounter,
        status: encounter.currentStatus
    };
}
function withEncounterViewAliases(encounter) {
    return {
...
async function lookupEncounterAppointmentMap(encounterIds) {
    ...
    for (const row of rows) {
        ...
        if (!encounterId || byEncounterId.has(encounterId))
            continue;
...
async function getClinicianProviderIds(userId) {
...
async function assertClinicInUserScope(user, clinic) {
...
async function assertEncounterInScope(encounter, user) {
    const clinic = await prisma.clinic.findUnique({
...
async function ensureRequiredFields(encounter, toStatus, overrideData) {
    const templateType = toStatus === "ReadyForProvider"
    ...
    const required = Array.isArray(template.requiredFields) ? template.requiredFields : [];
    const dataMap = overrideData ||
        (templateType === "rooming"
...
async function listEncountersForRole(filters) {
    ...
    const maUserIds = Array.from(new Set(encounters.map((encounter) => encounter.assignedMaUserId).filter((item) => Boolean(item))));
    const maUsers = maUserIds.length > 0
    ...
    return encounters.map((encounter) => withEncounterViewAliases({
        ...encounter,
...
export async function registerEncounterRoutes(app) {
    app.post("/encounters", { preHandler: requireRoles(RoleName.FrontDeskCheckIn, RoleName.Admin) }, async (request) => {
        ...
        const providerAssignedAndActive = clinic.maRun ||
        ...
        if (!clinicReady) {
            throw new ApiError(400, clinic.maRun
        ...
        let assignedMaUserId = clinicAssignment?.maUserId || undefined;
        const intakeData = incomingRecord?.intakeData ??
            (dto.intakeData ? dto.intakeData : null);
        if (reasonForVisitId && intakeData && typeof intakeData === "object") {
            ...
            if (intakeTemplate) {
                const required = Array.isArray(intakeTemplate.requiredFields)
                    ? intakeTemplate.requiredFields
                    : [];
                const missing = required.filter((field) => !intakeData[field]);
                if (missing.length > 0) {
    ...
    app.get("/encounters", { preHandler: requireRoles(RoleName.FrontDeskCheckIn, RoleName.MA, RoleName.Clinician, RoleName.FrontDeskCheckOut, RoleName.OfficeManager, RoleName.Admin) }, async (request) => {
        ...
        const scopedClinicId = requestedClinicId ||
    ...
    app.patch("/encounters/:id/status", { preHandler: requireRoles(RoleName.FrontDeskCheckIn, RoleName.MA, RoleName.Clinician, RoleName.FrontDeskCheckOut, RoleName.Admin) }, async (request) => {
        ...
        if (dto.toStatus === "Rooming")
            updates.roomingStartAt = encounter.roomingStartAt ?? new Date();
        if (dto.toStatus === "ReadyForProvider")
            updates.roomingCompleteAt = encounter.roomingCompleteAt ?? new Date();
        if (dto.toStatus === "Optimizing")
            updates.providerStartAt = encounter.providerStartAt ?? new Date();
        if (dto.toStatus === "CheckOut")
            updates.providerEndAt = encounter.providerEndAt ?? new Date();
    ...
    app.post("/encounters/:id/assign", { preHandler: requireRoles(RoleName.Admin) }, async (request) => {
        ...
        if (dto.assignedMaUserId) {
            ...
            const hasScopedMaRole = maUser.roles.some((entry) => {
                if (entry.role !== RoleName.MA)
                    return false;
                if (!clinic.facilityId)
                    return true;
                if (entry.facilityId)
                    return entry.facilityId === clinic.facilityId;
    ...
    app.post("/encounters/:id/checkout/complete", { preHandler: requireRoles(RoleName.FrontDeskCheckOut, RoleName.Admin) }, async (request) => {
        ...
        if (dto.checkoutData !== undefined) {
            data.checkoutData = dto.checkoutData;
        }
        const updated = await prisma.encounter.update({
            where: { id: encounterId },
            data
