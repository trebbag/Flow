...
function scopedFacilityIds(request) {
...
async function resolveFacilityForRequest(request, requestedFacilityId) {
    const user = request.user;
...
async function resolveClinicForFacility(clinicId, facilityId) {
...
function normalizeTemplateType(input) {
    ...
    if (value === "checkin" || value === "intake")
        return TemplateType.intake;
    if (value === "rooming")
        return TemplateType.rooming;
    if (value === "clinician")
        return TemplateType.clinician;
    if (value === "checkout")
        return TemplateType.checkout;
    throw new ApiError(400, `Unsupported template type '${input}'`);
}
function uiTemplateType(type) {
...
async function ensureFacilityThresholdDefaults(facilityId) {
...
function thresholdSortRank(metric, status) {
    if (metric === AlertThresholdMetric.overall_visit)
        return 999;
    const idx = thresholdStageOrder.findIndex((entry) => entry === status);
    return idx === -1 ? 500 : idx;
}
function isRoomNumberConstraintError(error) {
...
async function nextRoomNumberForFacility(facilityId, tx = prisma) {
...
async function restoreRoomWithAllocatedNumber(roomId) {
    for (let attempt = 0; attempt < 3; attempt += 1) {
        try {
            return await prisma.$transaction(async (tx) => {
...
function buildTemplateSchemas(fields) {
    const properties = {};
    const requiredFields = [];
    fields.forEach((field) => {
        const definition = field.type === "checkbox"
...
function reasonStatusToActive(status) {
    return status === "active";
}
function templateStatusToActive(status) {
    return status === "active";
}
...
async function listUserAssignmentImpact(userId) {
    ...
    const clinics = impactedClinics.map((clinic) => {
        ...
        const providerReady = clinic.maRun ||
...
export async function registerAdminRoutes(app) {
    app.get("/admin/clinics", { preHandler: requireRoles(RoleName.FrontDeskCheckIn, RoleName.MA, RoleName.Clinician, RoleName.FrontDeskCheckOut, RoleName.OfficeManager, RoleName.Admin, RoleName.RevenueCycle) }, async (request) => {
        ...
        const roomIdsByClinic = new Map();
        const activeRoomCountByClinic = new Map();
        roomAssignments.forEach((assignment) => {
            if (!roomIdsByClinic.has(assignment.clinicId))
                roomIdsByClinic.set(assignment.clinicId, []);
        ...
        return clinics.map((clinic) => {
            ...
            const providerAssignedActive = !clinic.maRun &&
                !!assignment?.providerUserId &&
                assignment.providerUser?.status === "active";
            const maAssignedActive = !!assignment?.maUserId &&
    ...
    app.post("/admin/clinics", { preHandler: requireRoles(RoleName.Admin) }, async (request) => {
        ...
        const created = await prisma.clinic.create({
            data: {
                ...
                cardTags: dto.cardTags ? dto.cardTags : Prisma.JsonNull
            }
        });
    ...
    app.post("/admin/clinics/:id", { preHandler: requireRoles(RoleName.Admin) }, async (request) => {
        ...
        const updated = await prisma.clinic.update({
            ...
            data: {
                ...
                autoCloseTime: dto.autoCloseTime,
                cardColor: dto.cardColor,
                cardTags: dto.cardTags ? dto.cardTags : undefined
            }
        });
        if (dto.roomIds) {
    ...
    app.post("/admin/reasons", { preHandler: requireRoles(RoleName.Admin) }, async (request) => {
        ...
        assert(clinics.every((clinic) => clinic.facilityId === facility.id && clinic.status !== "archived"), 400, "Visit reasons can only be assigned to non-archived clinics in the selected facility");
        const status = dto.status ?? (dto.active === false ? "inactive" : "active");
    ...
    app.post("/admin/reasons/:id", { preHandler: requireRoles(RoleName.Admin) }, async (request) => {
        const reasonId = request.params.id;
    ...
    app.get("/admin/rooms", { preHandler: requireRoles(RoleName.FrontDeskCheckIn, RoleName.MA, RoleName.Clinician, RoleName.FrontDeskCheckOut, RoleName.OfficeManager, RoleName.Admin) }, async (request) => {
        ...
        const statuses = includeArchived
            ...
            : includeInactive
                ? ["active", "inactive"]
                : ["active"];
        const assignmentWhere = query.clinicId
        ...
        const encounterCounts = roomIds.length > 0
            ...
            : [];
        const countsByRoomId = new Map(encounterCounts
            .filter((item) => item.roomId)
            .map((item) => [item.roomId, item._count._all]));
        const clinicIdsByRoomId = new Map();
        assignments.forEach((assignment) => {
            if (!clinicIdsByRoomId.has(assignment.roomId))
                clinicIdsByRoomId.set(assignment.roomId, []);
    ...
    app.post("/admin/rooms/reorder", { preHandler: requireRoles(RoleName.Admin) }, async (request) => {
        ...
        assert(currentIds.length === nextIds.length &&
            currentIds.every((entry, index) => entry === nextIds[index]), 400, "Reorder payload must include all active/inactive rooms for the selected facility");
        await prisma.$transaction(async (tx) => {
    ...
    app.post("/admin/assignment-overrides/:id/revoke", { preHandler: requireRoles(RoleName.Admin) }, async (request) => {
        const overrideId = request.params.id;
    ...
    function mapTemplateRow(template) {
        const reasonIds = Array.from(new Set([
    ...
    function deriveFieldsFromLegacyPayload(dto) {
        const source = dto.jsonSchema?.properties;
        ...
        const fields = Object.entries(source || {}).map(([key, value], index) => {
            ...
            const fieldType = rawType === "boolean"
                ? "checkbox"
                : rawType === "number"
                    ? "number"
    ...
    app.get("/admin/templates", { preHandler: requireRoles(RoleName.FrontDeskCheckIn, RoleName.MA, RoleName.Clinician, RoleName.FrontDeskCheckOut, RoleName.OfficeManager, RoleName.Admin) }, async (request) => {
        ...
        return templates.map((template) => mapTemplateRow(template));
    });
    async function upsertTemplate(params) {
        ...
        assert(reasons.every((reason) => reason.facilityId === facility.id && reason.status !== "archived"), 400, "Templates can only be assigned to non-archived reasons in the selected facility");
    ...
    app.delete("/admin/templates/:id", { preHandler: requireRoles(RoleName.Admin) }, async (request) => {
        ...
        return { status: "archived", template: mapTemplateRow(archived) };
    });
    ...
    app.get("/admin/thresholds", { preHandler: requireRoles(RoleName.Admin) }, async (request) => {
        ...
        return rows.sort((a, b) => {
            ...
            if (aOverrideRank !== bOverrideRank)
                return aOverrideRank - bOverrideRank;
    ...
    app.post("/admin/thresholds/:id", { preHandler: requireRoles(RoleName.Admin) }, async (request) => {
        ...
        const normalizedStatus = metric === AlertThresholdMetric.overall_visit
            ? null
    ...
    app.post("/admin/thresholds/bulk", { preHandler: requireRoles(RoleName.Admin) }, async (request) => {
        ...
        assert(facilityIdSet.size === 1 && facilityIdSet.has(facility.id), 400, "Bulk threshold update must target one facility");
        const clinicIds = Array.from(new Set(dto.rows
        ...
        await prisma.$transaction(async (tx) => {
            for (const existing of existingRows) {
                ...
                const status = metric === AlertThresholdMetric.overall_visit
                    ? null
    ...
    app.post("/admin/integrations/athenaone", { preHandler: requireRoles(RoleName.Admin) }, async (request) => {
        ...
        const normalizedConfig = JSON.parse(JSON.stringify(mergedConfig));
        const normalizedMapping = JSON.parse(JSON.stringify(dto.mapping || existing?.mappingJson || {}));
    ...
    app.get("/admin/notifications", { preHandler: requireRoles(RoleName.Admin) }, async (request) => {
        const query = request.query;
        const facility = await resolveFacilityForRequest(request, query.facilityId);
        const clinicIds = (await prisma.clinic.findMany({
            where: { facilityId: facility.id },
            select: { id: true }
    ...
    app.post("/admin/notifications", { preHandler: requireRoles(RoleName.Admin) }, async (request) => {
        ...
        return prisma.notificationPolicy.create({
            data: {
                ...
                recipientsJson: dto.recipients,
                channelsJson: dto.channels,
    ...
    app.post("/admin/notifications/:id/test", { preHandler: requireRoles(RoleName.Admin) }, async (request) => {
        ...
        const recipients = (Array.isArray(policy.recipientsJson) ? policy.recipientsJson : [])
            ...
            .filter((entry) => Object.values(RoleName).includes(entry));
    ...
    app.post("/admin/users/:id", { preHandler: requireRoles(RoleName.Admin) }, async (request) => {
        ...
        const shouldIncludeImpact = typeof dto.status === "string" &&
    ...
    app.delete("/admin/users/:id", { preHandler: requireRoles(RoleName.Admin) }, async (request) => {
        ...
        await prisma.$transaction(async (tx) => {
            ...
            const providerIds = Array.from(new Set(providerLinks
    ...
    app.post("/admin/users/:id/roles/remove", { preHandler: requireRoles(RoleName.Admin) }, async (request) => {
        ...
        const idsToDelete = roles
            .filter((role) => {
            if (dto.clinicId && role.clinicId === dto.clinicId)
                return true;
            if (dto.facilityId && role.facilityId === dto.facilityId)
                return true;
            if (!dto.clinicId && !dto.facilityId)
                return true;
            return false;
        })
            .map((role) => role.id);
