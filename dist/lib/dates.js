...
export function normalizeDate(date, timezone) {
...
export function parseAppointmentAt(appointmentTimeRaw, dateOfService, clinicTimezone) {
    const normalized = appointmentTimeRaw.trim();
    if (!normalized) {
        return { appointmentTime: null, appointmentAt: null, error: "Missing appointment time" };
    }
    const dateIso = DateTime.fromJSDate(dateOfService, { zone: "utc" }).setZone(clinicTimezone).toISODate() ||
