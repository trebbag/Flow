    ...
    if (validated.isValid) {
      const entry = await prisma.incomingSchedule.create({
        data: {
          ...
          intakeData: row.intakeData as Prisma.InputJsonValue,
          source,
...
export async function registerIncomingRoutes(app: FastifyInstance) {
  ...
  app.post("/incoming/pending/:id/retry", { preHandler: requireRoles(RoleName.FrontDeskCheckIn, RoleName.Admin) }, async (request) => {
    ...
    const clinicId = dto.clinicId || issue.clinicId || (issue.normalizedJson as Record<string, unknown> | null)?.clinicId as string | undefined;
    ...
    const normalized = (issue.normalizedJson as Record<string, unknown> | null) || {};
    const rawPayload = (issue.rawPayloadJson as Record<string, unknown> | null) || {};
    const facilityTimezone = facility.timezone || "America/New_York";
    const candidate: IncomingImportRow = {
      ...
      dateOfService:
        dto.dateOfService ??
        (normalized.dateOfService as string | null) ??
        ...
        null,
      appointmentTime: dto.appointmentTime ?? (normalized.appointmentTime as string | null) ?? (rawPayload.appointmentTime as string | null) ?? null,
      providerLastName:
        dto.providerLastName ??
        (normalized.providerLastName as string | null) ??
        (rawPayload.providerLastName as string | null) ??
        (rawPayload.providerName as string | null) ??
        null,
      reasonForVisit:
        dto.reasonText ??
        (normalized.reasonText as string | null) ??
    ...
    const created = await prisma.$transaction(async (tx) => {
      const incoming = await tx.incomingSchedule.create({
        data: {
          ...
          reasonForVisitId: validated.reasonForVisitId,
          reasonText: validated.reasonText,
          intakeData: (candidate.intakeData || {}) as Prisma.InputJsonValue,
  ...
  app.post("/incoming/:id/intake", { preHandler: requireRoles(RoleName.FrontDeskCheckIn, RoleName.MA, RoleName.Clinician, RoleName.Admin) }, async (request) => {
    ...
    return prisma.incomingSchedule.update({
      where: { id: incomingId },
      data: {
        intakeData: dto.intakeData as Prisma.InputJsonValue
      }
    });
  });
      ...
      if (!existing) {
        const encounter = await tx.encounter.create({
          data: {
            ...
            walkIn: false,
            intakeData: incoming.intakeData as Prisma.InputJsonValue,
            statusEvents: {
              create: {
                fromStatus: null,
                toStatus: "Optimized",
