...
export type RoomScopedClinic = { id: string; timezone: string; facilityId?: string | null };
...
function emptyStatusMinutes(): StatusMinutes {
  return Object.fromEntries(roomStatuses.map((status) => [status, 0])) as StatusMinutes;
}
...
function emptyIssueAccumulator() {
  ...
  ) as Record<RoomIssueType, IssueRollupRaw>;
}
...
function parseStatusMinutes(value: Prisma.JsonValue | null | undefined) {
  const minutes = emptyStatusMinutes();
  const raw = asRecord(value);
  if (!raw) return minutes;
...
function parseRoomRollups(value: Prisma.JsonValue | null | undefined): RoomRollupRaw[] {
  return asArray(value).map((entry) => {
    ...
    return {
      ...
      statusMinutes: parseStatusMinutes(row.statusMinutes as Prisma.JsonValue),
...
function parseIssueRollups(value: Prisma.JsonValue | null | undefined): IssueRollupRaw[] {
  return asArray(value)
    .map((entry) => {
      const row = entry as Record<string, unknown>;
      const issueType = row.issueType as RoomIssueType;
      if (!issueTypes.includes(issueType)) return null;
      return {
        issueType,
    ...
    .filter(Boolean) as IssueRollupRaw[];
}
...
function addStatusMinutes(target: StatusMinutes, status: RoomOperationalStatus, minutes: number) {
  if (minutes <= 0) return;
...
export async function computeRoomDailyRollup(
  prisma: PrismaClient,
  clinic: RoomScopedClinic,
  dateKey: string
): Promise<RoomDailyRollupRaw> {
  ...
  checklistRuns.forEach((run) => {
    ...
    if (!room) return;
    if (run.kind === "DayStart") room.dayStartCompleted = true;
    if (run.kind === "DayEnd") room.dayEndCompleted = true;
  });
  ...
  roomRows.forEach((room) => {
    ...
    let cursor = startUtc;
    let currentStatus =
      roomEvents.find((event) => event.occurredAt >= startUtc)?.fromStatus ||
    ...
    const recordInterval = (from: Date, to: Date, status: RoomOperationalStatus | null) => {
      if (!status) return;
...
export async function upsertRoomDailyRollup(prisma: PrismaClient, rollup: RoomDailyRollupRaw) {
...
function parseStoredRoomRollup(row: RoomDailyRollup): RoomDailyRollupRaw {
  return {
...
function mergeRoomRollups(dateKey: string, rows: RoomDailyRollupRaw[]): RoomDailyHistoryRollupView {
  const statusMinutes = emptyStatusMinutes();
  const issues = emptyIssueAccumulator();
  const rooms = new Map<string, RoomRollupRaw>();
  let roomCount = 0;
...
export async function getRoomDailyHistoryRollups(
  prisma: PrismaClient,
  clinics: RoomScopedClinic[],
  dateKeys: string[],
  options: RoomHistoryOptions = {}
) {
  const persist = options.persist ?? true;
  const forceRecompute = options.forceRecompute ?? false;
  const byClinicDate = new Map<string, RoomDailyRollupRaw>();
